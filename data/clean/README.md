...
These files are generated by the QC script after cleaning. See [`qc/README.md`](../../qc/README.md)
and [`qc/qc-microhabitat-observations.R`](../../qc/qc-microhabitat-observations.R) for details.

- [`microhabitat_observations_qc_issue_summary.csv`](microhabitat_observations_qc_issue_summary.csv): Summary of QC flags — counts of SUSPECT and REJECT records, or confirmation that all records passed.
- [`diagnostics/microhabitat_qc_flags.csv`](diagnostics/microhabitat_qc_flags.csv): Per-row QC flag results with flag type and reason for each flagged record.
- [`diagnostics/microhabitat_qc_flag_reason_counts.csv`](diagnostics/microhabitat_qc_flag_reason_counts.csv): Counts of flagged records grouped by flag type and reason.
- [`diagnostics/microhabitat_qc_report.rds`](diagnostics/microhabitat_qc_report.rds): Full QC results object for programmatic review.
